...
test:
	bash -c "docker run -it --rm -u root --name ${IMAGE_NAME}-${IMAGE_VERSION}-${NEW_VERSION_HASH} ${DOCKER_REGISTRY_WORKSTATION}${IMAGE_NAME}:${IMAGE_VERSION}-${NEW_VERSION_HASH}  bash"
	@printf "\n--> Test was done:\n\
			\t${DOCKER_REGISTRY_WORKSTATION}${IMAGE_NAME}:${IMAGE_VERSION}-${NEW_VERSION_HASH}\n"
